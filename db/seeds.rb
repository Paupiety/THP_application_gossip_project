...
gossips = []
20.times do
    gossip = Gossip.create(title: Faker::Book.title, description: Faker::Lorem.paragraph(sentence_count:5), user: users.sample)
    gossips << gossip
end
...
tags = []
10.times do
    tag = Tag.create(name: Faker::Lorem.word, user: users.sample)
    tags << tag
end
